...
общее время $t = \frac{5 * 1024 * 8 Кбит}{200 Кбит\сек} = 204,8$, но можно учесть, что данные будут проходить еще по двум
каналам на них мы потратим $\frac{5 * 8}\{3} + \frac{5 * 8}\{2} = 33,3$.
...
#### Решение
Скажем что в силу пропускной способности у нас не может быть больше 34 пользователей т.к. $35 * 100\frac{Кбит}{c} > 2\frac{Мбит}{c}$.
...
коммутатора и общее время передачи одного покета от хоста до хоста: $\frac{80 + S}{R}*(\frac{X}{S}-1) + 2 * \frac{80 + S}{R}$.
Возьмем у этого дела производную и получим $-\frac{80 * X}{S * R} + 2 = 0$. Отсюда $S =\sqrt{\frac{40 * X}{R}}$
