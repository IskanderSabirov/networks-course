...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - ru-RU, en
   - запущенная плафторма и браузер
   - кодировка (скорее сжатие -- zip, deflate)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - 172.23.51.241 - компьютера
   - 128.119.245.12 - сервера
...
5. Когда HTML-файл, который вы извлекаете, последний раз модифицировался на сервере?
   - Thu, 01 May 2025 05:59:01 GMT
6. Сколько байтов контента возвращается вашему браузеру?
   - 128 байт

<img src="images/img_1.png" width=2517  alt=""/>
<img src="images/img_2.png" width=1338  alt=""/>
...
1. Проверьте содержимое первого HTTP-запроса GET. Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET?
   - в первый - нет, во второй - содержит "If-Modified-Since: Thu, 01 May 2025 05:59:01 GMT"
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - В первый раз вернул явно 371 байт и "Line-based text data: text/html" с 10 линиями
   - Во второй раз вернулся статус "304 Not Modified",
...
<img src="images/img_3.png" width=2519  alt=""/>
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 4 сегмента: #695(1363), #696(1363), #697(1363), #698(772)
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Описано в пункте 3
<img src="images/img_4.png" width=2518 alt=""/>
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 запроса (2 доп на подгрузку png и jpg)
   - 128.119.245.12
   - 129.119.245.12
   - 178.79.137.164
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - на самом деле нет прчиин загружать их последовательно, ведь они запрашиваются с разных адресов, как минимум
      (по пакетам я вижу что мы сначала запросили - получили, запросили - получили)

<img src="images/img_5.png" width=2519 alt=""/>
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Authorization: Basic .... Credentials: wireshark-students:network
