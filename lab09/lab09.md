...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   - Назначения: 2.18.53.207
   - Хоста: 172.23.51.241
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - В силу того, что это протокол сетевого уровня, а не транспортного
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - тип 8, код 0
   - checksum, Identifier (BE,LE), Seqnumber (BE,LE), timestamp, data
   - по 2 байта

<img src="images/img.png" width=1967 alt=""/>
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - изменился тип на TTL
<img src="images/img_2.png" width=2018 alt=""/>
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - данные IPv4
<img src="images/img_3.png" width=2492 alt=""/>
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - тип 0 (Echo (ping) 0), не вижу поля ICMP исходного запроса и IPv4
<img src="images/img_4.png" width=2461 alt=""/>
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, есть резкий прыжок связанный с меддконтинентальной связью.
   - Пробил их по IP так сказать и показывает нидерланды и швецию.
<img src="images/img_5.png" width=2469 alt=""/>
...
1. $L$ - это у нас кол-во потерянных делить на кол-во отправленных.
Поэтому мы хотим найти число отправленных пакетов, мы считаем, что у нас медленный старт раз
мы изменяем скорость с $\dfrac{W}{2 * RTT}$ до $\dfrac{W}{RTT}$.
$$\dfrac{W}{2} + ( \dfrac{W}{2} + 1 ) + ... + W = \dfrac{3 * W^2}{8} + \dfrac{3 * W}{4}$$ .
Отсюда получаем необходимое.

2. Скажем что средняя скорость равна $MSS * W_{mea}n  / RTT$. Осталось найти $W_mean = \dfrac{3}{4} * W$. Из формулы выше мы получаем, что
$\sqrt{\dfrac{1}{L}} = \dfrac{3}{4} * 1.22$, подставив мы получаем что требовалось найти.
...
1. Посчитаем число отправленных пакетов:
$P = \dfrac{w}{2}* ( 1 + a^2 + ... + a^n ) = \dfrac{w}{2} * \dfrac{2a - 1}{a-1}$

Отсюда мы получаем обратное $L = \dfrac{2a - 2}{(2a -1)w}$

2. Заметим что нам потребуется $log_{1+a}2 * RTT$ времени для увеличения размера окна с $\dfrac{W}{2}$ до $W$.
А данная величина не зависит от средней пропускной способности TCP
($\dfrac{MSS}{L * (k + 1) * RTT}$)
