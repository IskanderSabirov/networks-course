...
<img src="images/img.png" width=2518 alt="" />
...
2. Найдите заголовок IP-пакета. Какое значение указано здесь в поле протокола верхнего уровня?
   - 17
<img src="images/img_1.png" width=1993 alt="" />
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - src, dst, protocol, total length, differentiated
      - version, services field, header length
      - TTL, identification, header checksum
...
<img src="images/img_3.png" width=2000 alt="" />
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification отличаются, TTL не отличаются
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - при установке такого размера пакета ломается, но если ставить значение в 1400.
        Почитал в интернете и на форумах чутка и пишут, что с флагом DF (dont fragment), а как отключить его на wsl я не знаю :(
        Но раз такой флаг не проходит, что можно заявить, что фрагментов >1 и раз его размер позволяет 1400, но не позволяет, то
        скорее всего их будет 3 ([3500/1500] = 3).

   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Смею предположить, что точно offset checksum, что по остальным надо на практике уже.
