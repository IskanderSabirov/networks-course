...
$\dfrac{F}{min{\dfrac{u_s}{N}, d_i}}$, максимальное значение времени будет достигаться при $d_i = d_{min}$
...
$\dfrac{F}{d_i} \leq \dfrac{F}{d_{min}}$, то есть мы не можем потратить время меньшее чем $\dfrac{F}{d_{min}}$.
...
Так мы получили, что время передачи $\geq max{\dfrac{N* f}{u_s}, \dfrac{F}{d_{min}}}$, теперь обратимся к пунктам 1 и 2
этой задачи и в зависимости от знак в сравнении $\dfrac{u_s}{N}$ и $d_{min}$ добьемся нужного времени.
