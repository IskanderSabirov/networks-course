...
#### Решение
Время потраченное на увеличение окна -- $\dfrac{W/2}{1MSS}$, причем средняя скорость равна $\dfrac{3W}{4}$, а отсюда уже можно
выразить $\dfrac{2S}{3MSSS}$
...
всего -- $2 * RTT + \dfrac{S}{R}$, увеличиваем окно в 2. Получаем первый сегмент этого окна за $\dfrac{S}{R}$ + $RTT$ на ACK,
-> делаем х2 у окна и повторяем процедуру и так же делаем х2 у окна 2 раза в конце получая окно размера 8MSS.
Тогда суммарно нам понадобится $2 * RTT + \dfrac{S}{R} + RTT + \dfrac{S}{R} + RTT + \dfrac{12S}{R}$
...
$2 * RTT + \dfrac{S}{R} + RTT + \dfrac{S}{R} + RTT + \dfrac{S}{R} + RTT + \dfrac{8S}{R}$

3. Начало как выше, но в виду ограничения неравенства окна будут идти подряд и получаем
$2 * RTT + \dfrac{S}{R} + RTT + \dfrac{S}{R} + \dfrac{13S}{R}$.
...
$\dfrac{W}{2} + \dfrac{W}{2} * (1 + a) + \dfrac{W}{/2} * (1 + a)^2 + ... + \dfrac{W}{2} * (1 + a)^b)$,
причем $b = log_{1+a}(2)$  и получаем $Cnt = \dfrac{W * (2a + 1)}{2a} -> L = \dfrac{2 * a}{W * ( 2 * a + 1)}$
